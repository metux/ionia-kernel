...
static void evm_nand_init(int evm_id, int profile)
{
	...
	pr_info ("board-am335xevm.c: evm_nand_init() started\n");
	setup_pin_mux(nand_pin_mux);
	pr_info ("board-am335xevm.c: omap_nand_init() start\n");
	pdata = omap_nand_init(am335x_nand_partitions,
		ARRAY_SIZE(am335x_nand_partitions), 0, 0,
		&am335x_nand_timings);
	if (!pdata)
	{
		pr_err ("board-am335xevm.c: evm_nand_init() aborted\n");
		return;
	}
	...
	pr_info ("board-am335xevm.c: omap_init_gpmc() start\n");
	omap_init_gpmc(gpmc_device, sizeof(gpmc_device));
	pr_info ("board-am335xevm.c: omap_init_elm() start\n");
	omap_init_elm();
	pr_info ("board-am335xevm.c: evm_nand_init() finished\n");
}
